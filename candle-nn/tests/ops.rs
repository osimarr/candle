...
#[cfg(feature = "rocm")]
fn assert_close_f8e4m3(lhs: &Tensor, rhs: &Tensor, max_mean_diff: f32) -> Result<()> {
    assert_eq!(lhs.dtype(), DType::F8E4M3);
    assert_eq!(rhs.dtype(), DType::F8E4M3);
    let diff = (lhs.to_dtype(DType::F32)? - rhs.to_dtype(DType::F32)?)?
        .abs()?
        .sum_all()?
        .to_vec0::<f32>()?
...
#[cfg(feature = "rocm")]
#[test]
fn rocm_f8e4m3_callbacks() -> Result<()> {
    let device = Device::new_rocm(0)?;
    let data = &[[[3f32, 1., 4.], [1., 5., 9.]], [[2., 1., 7.], [8., 2., 8.]]];
    let tensor_f32 = Tensor::new(data, &device)?;
    let tensor = tensor_f32.to_dtype(DType::F8E4M3)?;

    let sigmoid = candle_nn::ops::sigmoid(&tensor)?;
    let sigmoid_expected =
        candle_nn::ops::sigmoid(&tensor.to_dtype(DType::F32)?)?.to_dtype(DType::F8E4M3)?;
    assert_close_f8e4m3(&sigmoid, &sigmoid_expected, 0.02)?;

    let logits = tensor_f32.log()?.to_dtype(DType::F8E4M3)?;
    let softmax = candle_nn::ops::softmax_last_dim(&logits)?;
    let softmax_expected =
        candle_nn::ops::softmax_last_dim(&logits.to_dtype(DType::F32)?)?.to_dtype(DType::F8E4M3)?;
    assert_close_f8e4m3(&softmax, &softmax_expected, 0.02)?;

    let alpha = Tensor::new(&[1f32, 2f32, 3f32], &device)?.to_dtype(DType::F8E4M3)?;
    let beta = Tensor::new(&[0.5f32, 0f32, -0.2f32], &device)?.to_dtype(DType::F8E4M3)?;
    let rms = candle_nn::ops::rms_norm(&tensor, &alpha, 1e-5)?;
    let rms_expected = candle_nn::ops::rms_norm_slow(
        &tensor.to_dtype(DType::F32)?,
        &alpha.to_dtype(DType::F32)?,
    ...
    assert_close_f8e4m3(&rms, &rms_expected, 0.05)?;
    ...
    .to_dtype(DType::F8E4M3)?;
    assert_close_f8e4m3(&layer, &layer_expected, 0.05)?;
    ...
    let src = Tensor::from_vec(src_values, (b_size, num_head, seq_len, head_dim), &device)?
        .to_dtype(DType::F8E4M3)?;
    let cos =
        Tensor::from_vec(cos_values, (seq_len, head_dim / 2), &device)?.to_dtype(DType::F8E4M3)?;
    let sin =
        Tensor::from_vec(sin_values, (seq_len, head_dim / 2), &device)?.to_dtype(DType::F8E4M3)?;
    let src_f32 = src.to_dtype(DType::F32)?;
    let cos_f32 = cos.to_dtype(DType::F32)?;
    let sin_f32 = sin.to_dtype(DType::F32)?;

    let rope_i = candle_nn::rotary_emb::rope_i(&src, &cos, &sin)?;
    let rope_i_expected = candle_nn::rotary_emb::rope_i_slow(&src_f32, &cos_f32, &sin_f32)?
        .to_dtype(DType::F8E4M3)?;
    assert_close_f8e4m3(&rope_i, &rope_i_expected, 0.05)?;

    let rope = candle_nn::rotary_emb::rope(&src, &cos, &sin)?;
    let rope_expected =
        candle_nn::rotary_emb::rope_slow(&src_f32, &cos_f32, &sin_f32)?.to_dtype(DType::F8E4M3)?;
    assert_close_f8e4m3(&rope, &rope_expected, 0.05)?;

    let rope_thd = {
        let src = src.transpose(1, 2)?.contiguous()?;
        candle_nn::rotary_emb::rope_thd(&src, &cos, &sin)?.transpose(1, 2)?
    };
    assert_close_f8e4m3(&rope_thd, &rope_expected, 0.05)?;
