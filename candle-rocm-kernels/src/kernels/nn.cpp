...
__global__ void softmax_last_dim_f8e4m3_kernel(
    const uint8_t* src,
    size_t start_offset,
    uint8_t* dst,
    size_t rows,
...
__global__ void rms_norm_f8e4m3_kernel(
    const uint8_t* src,
    size_t src_start_offset,
    const uint8_t* alpha,
    size_t alpha_start_offset,
    ...
    const uint8_t* src_row = src + src_start_offset + row * cols;
    const uint8_t* alpha_row = alpha + alpha_start_offset;
    float sum2 = 0.0f;
    for (size_t col = 0; col < cols; ++col) {
        const float value = f8e4m3_bits_to_f32(src_row[col]);
        volatile float squared = value * value;
        sum2 += squared;
    }
    const float mean = sum2 * (1.0f / static_cast<float>(cols));
    ...
    for (size_t col = 0; col < cols; ++col) {
        const float value =
            (f8e4m3_bits_to_f32(src_row[col]) / denom) * f8e4m3_bits_to_f32(alpha_row[col]);
        dst_row[col] = f32_to_f8e4m3_bits(value);
    }
}
...
__global__ void layer_norm_f8e4m3_kernel(
    const uint8_t* src,
    size_t src_start_offset,
    const uint8_t* alpha,
    size_t alpha_start_offset,
    ...
    const uint8_t* src_row = src + src_start_offset + row * cols;
    const uint8_t* alpha_row = alpha + alpha_start_offset;
    const uint8_t* beta_row = beta + beta_start_offset;
    float sum = 0.0f;
    float sum2 = 0.0f;
    for (size_t col = 0; col < cols; ++col) {
        const float value = f8e4m3_bits_to_f32(src_row[col]);
    ...
    uint8_t* dst_row = dst + row * cols;
    for (size_t col = 0; col < cols; ++col) {
        const float value = (f8e4m3_bits_to_f32(src_row[col]) - mean) * inv_std *
                                f8e4m3_bits_to_f32(alpha_row[col]) +
                            f8e4m3_bits_to_f32(beta_row[col]);
        dst_row[col] = f32_to_f8e4m3_bits(value);
    }
}
...
__global__ void rope_f8e4m3_kernel(
    const uint8_t* src,
    size_t src_start_offset,
    const uint8_t* cos,
    size_t cos_start_offset,
    ...
    const size_t rope_offset =
        (unbatched_rope ? b_i * t * half_d : 0) + t_i * half_d + d_pair;
    const float c = f8e4m3_bits_to_f32(cos[cos_start_offset + rope_offset]);
    const float s = f8e4m3_bits_to_f32(sin[sin_start_offset + rope_offset]);
    if (interleaved) {
        const size_t i0 = src_start_offset + row_base + 2 * d_pair;
        const size_t i1 = i0 + 1;
        const float x0 = f8e4m3_bits_to_f32(src[i0]);
        const float x1 = f8e4m3_bits_to_f32(src[i1]);
        dst[row_base + 2 * d_pair] = f32_to_f8e4m3_bits(x0 * c - x1 * s);
        dst[row_base + 2 * d_pair + 1] = f32_to_f8e4m3_bits(x0 * s + x1 * c);
    } else {
        const size_t i0 = src_start_offset + row_base + d_pair;
        const size_t i1 = src_start_offset + row_base + d_pair + half_d;
        const float x0 = f8e4m3_bits_to_f32(src[i0]);
        const float x1 = f8e4m3_bits_to_f32(src[i1]);
        dst[row_base + d_pair] = f32_to_f8e4m3_bits(x0 * c - x1 * s);
        dst[row_base + d_pair + half_d] = f32_to_f8e4m3_bits(x0 * s + x1 * c);
    }
}
...
extern "C" int hip_softmax_last_dim_f8e4m3(
    int ordinal,
    const uint8_t* src,
    size_t start_offset,
    uint8_t* dst,
...
extern "C" int hip_layer_norm_f8e4m3(
    int ordinal,
    const uint8_t* src,
    size_t src_start_offset,
    const uint8_t* alpha,
